...
namespace Common.Helpers
{
    ...
    public static class RandomHelper
    {
        ...
        {
            var range = max - min;
            var strFormat = "{0:F" + digits + "}";
            // To avoid having to get an int and then add a random double ...
            var randDou = double.Parse(string.Format(strFormat, min + range*RandomSeed.NextDouble()));
            ...
            return randDou < max ? randDou : RandomNumber(min, max, digits);
        ...
        {
            var mold = "1234-{0}-{1}";
            return string.Format(mold, RandomInt(0, 1000).ToString().PadLeft(3, '0'),
                RandomInt(0, 1000).ToString().PadLeft(3, '0'));
        }
        ...
        /// <returns>
        ///     If both param are true or null, random name. Otherwise, male or female
        ///     name depending on the param that is true
        /// </returns>
        public static string RandomName(bool? aMale = null, bool? aFemale = null)
        {
                ...
                (aMale == true && aFemale == true))
            {
                ...
                first = gender
                    ? ListOfMaleFirstNames[RandomInt(0, MaleFirstNamesCount)]
            ...
            var last = ListOfLastNames[RandomInt(0, LastNamesCount)];
            return $"{first} {last}";
        }
    }
}